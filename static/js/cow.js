...
function refreshCows(){
  prompt = randomIntFromInterval(0, 3);
  var yes_or_no = randomIntFromInterval(0, 1);
  if (yes_or_no === 0) {
    prompt_text = no_prompts[prompt];
  }
  ...
  for(var i = 0; i < 4; i++){
    if (i === answer) {
      cows[i].setAttribute('onclick','movingOn()');
      if (yes_or_no === 0) {
        cows[i].src = './cows/n_'+randomIntFromInterval(1, 6)+'.png';
      }
      else {cows[i].src = './cows/y_'+randomIntFromInterval(1, 6)+'.png';}
    }
    else { cows[i].setAttribute('onclick','gameOver()');
      if (yes_or_no === 0) {
          cows[i].src = './cows/y_'+randomIntFromInterval(1, 6)+'.png';
        }
        else {cows[i].src = './cows/n_'+randomIntFromInterval(1, 6)+'.png';}
      }
  }
}
